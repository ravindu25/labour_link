...
function openRejectModal(bookingId){
    const backdropModal = document.getElementById('backdrop-modal');
    const rejectBookingContainer = document.getElementById('reject-booking-container');
    const rejectButton = document.getElementById('reject-confirm-button');
    ...
    backdropModal.style.visibility = 'visible';
    rejectBookingContainer.style.visibility = 'visible';
}
...
function closeRejectModal(){
    ...
    backdropModal.style.visibility = 'hidden';
    rejectBookingContainer.style.visibility = 'hidden';
}
...
/*
    Purpose - Reject selected booking
 */

async function rejectBooking(bookingId){
    const bookingRejectContent = document.getElementById('reject-booking-content');
    const bookingRejectButtons = document.getElementById('reject-booking-buttons');
    const loadingSpinnerContainer = document.getElementById('loader-container');

    bookingRejectButtons.style.display = 'none';
    bookingRejectContent.style.display = 'none';
    ...
    fetch(`http://localhost/labour_link/api/bookings.php?bookingId=${bookingId}`, {
        ...
        .then(() => {
            const bookingMessage = document.getElementById('reject-booking-text');

            loadingSpinnerContainer.style.display = 'none';
            bookingRejectContent.style.display = 'block';
            bookingMessage.innerHTML = `<i class="fa-solid fa-check"></i>&nbsp;&nbsp;Booking rejected successfully`;

            setTimeout(() => {
                closeRejectModal();
                window.location.href = "../worker/bookings.php";
            }, 5000);
        })
        .catch((error) => {
            const bookingMessage = document.getElementById('reject-booking-text');
            ...
            bookingRejectContent.style.display = 'block';
            bookingMessage.innerHTML = `<i class="fa-solid fa-xmark"></i>&nbsp;&nbsp;Booking rejection failed`;
